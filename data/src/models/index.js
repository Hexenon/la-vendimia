 ...
 * @returns {Promise}
 */
module.exports.prepare = (connectors)=>{
        ...
        async.eachSeries(Object.keys(connectors), (connectorName, cb)=>{
            ...
            switch (connector.type){
                case ConnectorTypes.sql:
                    require(__dirname + '/sql')(connector, (result)=>{
                        models = _.merge(models, result);
                        cb();
                    });
                    break;
                case ConnectorTypes.mongodb:
                    require(__dirname + '/mongodb')(connector, (result)=> {
                        models = _.merge(models, result);
                        cb();
                    });
                    break;
                case ConnectorTypes.dynamodb:
                    require(__dirname + '/dynamodb')(connector, (result)=> {
                        models = _.merge(models, result);
                        cb();
                    });
                    break;
